...
handle_cmd(derived_tool_or_menuitem, _, Frame) ->
    ...
    wxDialog:destroy(Pref), %% Added for testing (missing from original)
    ok;
...
prefDialog(Parent) ->    
    ...
    Text = wxXmlResource:xrcctrl(Dlg, "my_textctrl", wxTextCtrl), %% Moved
    ...
    wxTextCtrl:connect(Text, command_text_updated, [{callback, fun(_,_) -> io:format("command_text_updated~n") end}]),
