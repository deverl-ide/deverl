...
%% This is a feasibility test for XRC in the ide.
%% On OSX wx3.0.0 erlangR16B03, this demo segfaults when the derived dialog
%% is closed (and destroyed). Likely cause is events are sent to the object after
%% its destruction.
...
loop(Frame) ->
    receive 
  #wx{id=Id, event=#wxCommand{type=command_button_clicked}}=R ->
    io:format("btn_clicked ~p~n", [R]),
    loop(Frame);
...
%% This dialog crashes when closed with the close window button
%% The dialog has been moved to xrctest where it uses wx_object
%% behaviour.
handle_cmd(derived_tool_or_menuitem, _, Frame) ->
    Pref = prefDialog(Frame),
    case wxDialog:showModal(Pref) of
      ?wxID_OK ->
        io:format("Dialog returned OK~n",[]);
      ?wxID_CANCEL ->
        io:format("Dialog returned CANCEL~n",[])
    end,
    xrctest:destroy(Pref);
...
%% @see xrc_unmod for original working version
prefDialog(Parent) ->    
    Dlg = xrctest:start(Parent).
