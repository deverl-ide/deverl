...
get_selected_editor() ->
  ...
  Valid = fun(-1) -> %% no editor instance
          ...
          (_) ->
            ...
            {ok, {Index, get_editor(Index, Workspace), Workspace}}
          end,
  Valid(Index).   
...
save_current_file() ->
  case get_selected_editor() of
    ...
    {ok, {Index, Editor, Workspace}} ->
      save_file(Index, Editor, Workspace)
      % Contents = wxStyledTextCtrl:getText(Editor),
      % Filename = ide_io:save(Editor, Contents),
      % wxAuiNotebook:setPageText(Workspace,Index, Filename),
      % editor:save_complete(Editor)
  end.
  
save_file(Index, Editor, Workspace) ->
  % case editor:save_request(wx:typeCast(wxWindow:getParent(Editor),wxPanel)) of
  %   {ok, unsaved} ->
  %     io:format("file not saved~n");
  %   _ ->
  %     ok
  % end.
  Pid = wx_object:get_pid(wx:typeCast(wxWindow:getParent(Editor), wxPanel)).
