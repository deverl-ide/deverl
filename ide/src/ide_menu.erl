...
init(Config) ->
    ...
    wxMenu:append(Edit, ?wxID_SEPARATOR, []),
    wxMenu:append(Edit, 987987987, "Select All"),
    ...
    wxMenu:append(View, ?wxID_SEPARATOR, []),
    wxMenu:append(View, 65465431, "Scroll Past Last Line", [{kind, ?wxITEM_CHECK}]),
    wxMenu:check(View, 65465431, user_prefs:get_user_pref({pref, scroll_past_end})),
    wxMenu:append(View, ?wxID_SEPARATOR, []),
    ...
    wxMenu:append(Document, ?wxID_SEPARATOR, []),
    wxMenu:append(Document, 32154, "Uppercase Selection"),
    wxMenu:append(Document, 98745, "Lowercase Selection"),
    wxMenu:append(Document, ?wxID_SEPARATOR, []),
    ...
    wxMenu:append(ToolMenu, ?MENU_ID_DEBUGGER, "Run Debugger"),
	  ...
	  Window        = wxMenu:new([]),
	  wxMenu:append(Window, ?wxID_HELP, "Projects \tCtrl+1"),
	  wxMenu:append(Window, ?MENU_ID_SHORTCUTS, "Tests \tCtrl+2"),
	  wxMenu:append(Window, ?wxID_SEPARATOR, []),
	  wxMenu:append(Window, ?MENU_ID_SEARCH_DOC, "Console \tCtrl+3"),
	  wxMenu:append(Window, ?MENU_ID_MANUAL, "Observer \tCtrl+4"),
	  wxMenu:append(Window, ?MENU_ID_SEARCH_DOC, "Dialyser \tCtrl+5"),
	  wxMenu:append(Window, ?MENU_ID_MANUAL, "Debugger \tCtrl+6"),
	  wxMenu:append(Window, ?wxID_SEPARATOR, []),
	  wxMenu:append(Window, 654654, "Next Tab \tCtrl+}"),
	  wxMenu:append(Window, 321321, "Previous Tab \tCtrl+{"),
    ...
    ets:insert(TabId, [
    	...
    	{?MENU_ID_TESTS, {}, [{group, ?MENU_GROUP_ED bor ?MENU_GROUP_WS}]},
    	{?MENU_ID_DEBUGGER, {}, [{group, ?MENU_GROUP_ED bor ?MENU_GROUP_WS}]},
