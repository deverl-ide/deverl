...
-include("../include/observer_defs.hrl").
...
start(Config) ->
    case wx_object:start(?MODULE, Config, []) of
	Err = {error, _} -> Err;
	Obj -> Obj
    end.
...
init(Config) ->
    ...
    {Notebook, UpdState} = setup(State),
    net_kernel:monitor_nodes(true),
    process_flag(trap_exit, true),
    {Notebook, UpdState}.

setup(#state{parent=Frame}=State) ->
    %% Setup Menubar & Menus
    ...
    Notebook = wxAuiNotebook:new(Panel, []),

    %% System Panel
    SysPanel = observer_sys_wx:start_link(Notebook, self()),
    wxAuiNotebook:addPage(Notebook, SysPanel, "System", []),
    ...
    wxAuiNotebook:addPage(Notebook, PerfPanel, "Load Charts", []),

    %% App Viewer Panel
    AppPanel = observer_app_wx:start_link(Notebook, self()),
    wxAuiNotebook:addPage(Notebook, AppPanel, "Applications", []),

    %% Process Panel
    ProPanel = observer_pro_wx:start_link(Notebook, self()),
    wxAuiNotebook:addPage(Notebook, ProPanel, "Processes", []),

    %% Table Viewer Panel
    TVPanel = observer_tv_wx:start_link(Notebook, self()),
    wxAuiNotebook:addPage(Notebook, TVPanel, "Table Viewer", []),

    %% Trace Viewer Panel
    TracePanel = observer_trace_wx:start_link(Notebook, self()),
    wxAuiNotebook:addPage(Notebook, TracePanel, ?TRACE_STR, []),
