...
server(StartSync) ->
    ...
    io:format("Init bindings: ~p~n", [Bs]),
    ...
    io:format("Init records: ~p~n", [ets:tab2list(RT)]),
	...
	_undefined_or_false ->
      % io:fwrite(<<"Eshell V~s  (abort with ^G)\n">>,
	    io:fwrite(<<"Init Shell  V~s  (abort with ^G)\n">>,
		      [erlang:system_info(version)])
    end,
    erase(no_control_g),
...
server_loop(N0, Eval_0, Bs00, RT, Ds00, History0, Results0) ->
    io:format("Server loop - N0: ~p~nEval_0: ~p~nBs00: ~p~nRT: ~p~nDS00: ~p~nResults0: ~p~n",
      [N0, Eval_0, Bs00, ets:tab2list(RT), Ds00, Results0]),
    N = N0 + 1,
    ...
    Parse =
        fun() ->
                exit(
                  ...
                  of
                      {ok,Toks,_EndPos} ->
                          io:format("Get cmd parse exprs.~n"),
                          erl_parse:parse_exprs(Toks);
                      {eof,_EndPos} ->
                          io:format("Get cmd eof~n"),
                          eof;
                      {error,ErrorInfo,_EndPos} ->
                          io:format("Get cmd error~n"),
...
start_eval(Bs, RT, Ds) ->
    io:format("Start the evaluator~n"),
...
eval_loop(Shell, Bs0, RT) ->
    ...
    io:format("Eval loop - Shell~p~nBs0: ~p~nRT: ~p~nEs: ~p~nW: ~p~n",
      [Shell, Bs0, ets:tab2list(RT), Es, W]),
            Ef = {value, 
